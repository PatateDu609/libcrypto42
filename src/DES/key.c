/**
 * @file key.c
 * @author Ghali Boucetta (gboucett@student.42.fr)
 * @brief All the key management functions
 * @date 2022-08-14
 */

#include "internal.h"

/**
 * @brief Set the up key object. It will remove the parity bits and permute the key.
 *
 * @param key The key object to set up
 * @param left pointer to left key storage
 * @param right pointer to right key storage
 */
static uint64_t setup_key(uint64_t key) {
	const uint8_t pc_1[56] = {
		57, 49, 41, 33, 25, 17, 9,  1, 58, 50, 42, 34, 26, 18, 10, 2, 59, 51, 43, 35, 27, 19, 11, 3, 60, 52, 44, 36,
		63, 55, 47, 39, 31, 23, 15, 7, 62, 54, 46, 38, 30, 22, 14, 6, 61, 53, 45, 37, 29, 21, 13, 5, 28, 20, 12, 4,
	};

	return permute(key, 64, pc_1, 56);
}

void key_schedule(uint64_t key, uint64_t *subkeys) {
	key                    = setup_key(key);

	const uint8_t pc_2[48] = {
		14, 17, 11, 24, 1,  5,  3,  28, 15, 6,  21, 10, 23, 19, 12, 4,  26, 8,  16, 7,  27, 20, 13, 2,
		41, 52, 31, 37, 47, 55, 30, 40, 51, 45, 33, 48, 44, 49, 39, 56, 34, 53, 46, 42, 50, 36, 29, 32,
	};

	const uint8_t shifts[NB_ROUNDS] = { 1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1 };

	uint32_t      left, right;
	left  = key >> 28;
	right = key & OVERFLOW_MASK_28;
	for (int i = 0; i < NB_ROUNDS; i++) {
		// It is important to mask the overflow bits (as they will override the bits of the other half)
		left = ((left << shifts[i]) | (left >> (28 - shifts[i])));
		left &= OVERFLOW_MASK_28;
		right = ((right << shifts[i]) | (right >> (28 - shifts[i])));
		right &= OVERFLOW_MASK_28;

		subkeys[i] = (((uint64_t) left) << 28) | right;
		subkeys[i] = permute(subkeys[i], 56, pc_2, 48);
	}
}
