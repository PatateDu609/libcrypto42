...
TEST(AES128, key_expansion) {
	...
	expected.resize(AES_KEY_SCHEDULE_LENGTH, 0);

	key_expansion(&ctx, key.data());

	std::vector<uint32_t> actual(ctx.key_schedule, ctx.key_schedule + AES_KEY_SCHEDULE_LENGTH);
	EXPECT_EQ(actual, expected);
}

TEST(AES192, key_expansion) {
	struct aes_ctx ctx = {
		.type = AES192,
		.Nk   = AES192_KEY_SIZE,
		.Nb   = AES_BLK_SIZE,
	...
	std::array<uint32_t, 6> key{ 0x8e73b0f7, 0xda0e6452, 0xc810f32b, 0x809079e5, 0x62f8ead2, 0x522c6b7b };
	std::vector<uint32_t>   expected{
...
TEST(AES256, key_expansion) {
	...
	std::array<uint32_t, 8> key{ 0x603deb10, 0x15ca71be, 0x2b73aef0, 0x857d7781,
