...
Test(AES128, key_expansion) {
	...
	cr_expect(eq(u32[44], ctx.key_schedule, expected));
}
...
Test(AES192, key_expansion) {
	...
	cr_expect(eq(u32[52], ctx.key_schedule, expected));
}
...
Test(AES256, key_expansion) {
	...
	uint32_t key[] = { 0x603deb10, 0x15ca71be, 0x2b73aef0, 0x857d7781, 0x1f352c07, 0x3b6108d7, 0x2d9810a3, 0x0914dff4 };
	uint32_t expected[] = {
	...
	cr_expect(eq(u32[60], ctx.key_schedule, expected));
}