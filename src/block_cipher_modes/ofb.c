...
static inline uint8_t *perform_OFB(struct cipher_ctx *ctx, struct blk *in, struct blk *out) {
	...
	for (size_t idx = 0; idx < input_size_bits; idx += s) {
		...
		struct blk *output_block_s_bits_duped = block_dup(output_block_s_bits);
