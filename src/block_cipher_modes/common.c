...
void block_xor(struct blk *res, const struct blk *a, const struct blk *b) {
	size_t min_size = res->len;
...
void block_left_shift(struct blk *a, size_t n) {
	size_t q, r;

	q = n / 8;
	if (q >= a->len) {
		memset(a->data, 0, a->len * sizeof *a->data);
		return;
	}
	...
	if (q) {
		memmove(a->data, a->data + q, (a->len - q) * sizeof *a->data);
		memset(a->data + (a->len - q), 0, q * sizeof *a->data);
	}
	...
	if (r) {
		...
		for (size_t i = 0; i < a->len; i++) {
			...
			uint8_t next    = i + 1 < a->len ? a->data[i + 1] : 0;
...
void block_right_shift(struct blk *a, size_t s) {
	if (!s)
		return;
	...
	if (quotient) {
		memmove(a->data + quotient, a->data, (a->len - quotient) * sizeof *a->data);
	...
	for (ssize_t i = (ssize_t) a->len - 1; i >= (ssize_t) quotient; i--) {
		uint8_t current = a->data[i];
...
struct blk *block_bit_extract(const struct blk *blk, size_t sub) {
	struct blk *res = calloc(1, sizeof *res);
	if (res == NULL)
		return NULL;
	...
	res->len = blk->len;
	res->data = calloc(res->len, sizeof *res->data);
	if (res->data == NULL) {
		free(res);
		return NULL;
	}
	...
	extracted_size = (extracted_size > blk->len) ? blk->len : extracted_size;
...
void block_bit_assign(struct blk *res, struct blk *src, size_t start, size_t nb) {
	size_t byte_start = start / 8;
	if (res->len <= byte_start) {
		return;
	}
	...
	if (res->len < byte_nb + byte_start)
		byte_nb = res->len - byte_start;

	if (byte_nb)
		memcpy(res->data + byte_start, src->data + byte_start, byte_nb * sizeof *src->data);

	size_t byte_remaining = res->len - byte_nb - byte_start;
	if (!byte_remaining)
		return;
...
void block_increment(struct blk *blk, size_t bit_limit) {
	int32_t tmp;
	size_t  computed     = 0;
	...
	for (ssize_t i = (ssize_t) (blk->len - 1); i >= 0 && computed < bit_limit; i--, computed += 8) {
...
struct blk *block_dup(const struct blk *src) {
	struct blk *res = calloc(1, sizeof *res);
	if (!res)
		return NULL;
	...
	if (!(res->data = calloc(res->len, sizeof *res->data))) {
		free(res);
		return NULL;
	}

	memcpy(res->data, src->data, res->len * sizeof *res->data);
	return res;
}

struct blk *block_dup_data(uint8_t *data, size_t size) {
	struct blk blk;
	blk.len = size;
	blk.data = data;
...
struct blk *block_create(size_t size) {
	struct blk *res = calloc(1, sizeof *res);
	if (!res)
		return NULL;
...
void block_delete(struct blk *blk) {
	if (blk) {
		free(blk->data);
		free(blk);
	}
...
void block_encrypt(const struct cipher_ctx *ctx, struct blk *res, const struct blk *a) {
	...
	memcpy(res->data, fn_res, res->len);
	free(fn_res);
}

void block_decrypt(const struct cipher_ctx *ctx, struct blk *res, const struct blk *a) {
	res->len                                               = a->len;
	static uint8_t *(*alg_op[])(uint8_t *, const uint8_t *) = {
